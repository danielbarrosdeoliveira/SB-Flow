      ...
      <template #header="{ collapsed }">
        <div class="flex items-center gap-2 px-2">
          <div class="w-8 h-8 rounded-lg bg-primary flex items-center justify-center shrink-0">
            <span class="text-sm font-bold text-primary-950">SB</span>
          </div>
          <span v-show="!collapsed" class="font-serif text-lg text-primary-light">Flow</span>
        </div>
      </template>
    ...
    <div class="flex-1 flex flex-col min-w-0">
      <header class="h-16 flex items-center justify-between px-4 lg:px-6 shrink-0">
        <div class="flex items-center gap-2">
          ...
          <h1 class="text-lg font-semibold truncate text-foreground">{{ pageTitle }}</h1>
        </div>
        <div class="flex items-center gap-2">
          <UButton icon="i-lucide-bell" color="neutral" variant="ghost" :square="true" />
          <UColorModeButton />
          <span class="text-sm hidden sm:inline text-muted">{{ userName }}</span>
        </div>
      </header>
